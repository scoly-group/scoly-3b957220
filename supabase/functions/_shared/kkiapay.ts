...
export async function settleTransaction(
  admin: Admin,
  params: { transactionId: string; orderId?: string | null; verification: KkiapayVerification },
) {
  ...
  // inférieure au total de la commande ne peut jamais la confirmer.
  const orderTotal = Number(order.total_amount ?? 0);
  const paidAmount = Number(verification.amount ?? 0);
  const amountOk = paidAmount + 1 >= orderTotal;
  const settlementOk = verification.ok && amountOk;
  if (verification.ok && !amountOk) {
    console.error(
      `[kkiapay] montant insuffisant: transaction ${transactionId} = ${paidAmount}, commande ${orderId} = ${orderTotal}`,
    );
  }
  ...
  if (!settlementOk) {
    ...
    return {
      ok: false,
      error: (verification.ok ? "amount_mismatch" : "payment_not_successful") as const,
      orderId,
    };
  }
  ...
  if (!paymentId) {
    const { data: created, error } = await admin
      ...
      .insert({
        ...
        amount: paidAmount || order.total_amount,
        subtotal_amount: paidAmount || order.total_amount,
        payment_method: method,
        status: "pending",
        transaction_id: transactionId,
        payment_reference: transactionId,
        phone_number: order.phone,
        metadata: { provider: "kkiapay", method },
      })
      .select("id")
      .single();
