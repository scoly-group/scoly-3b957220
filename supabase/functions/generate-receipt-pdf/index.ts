...
function buildPdf(order: any, items: any[], customer: { name: string; phone: string; address: string }) {
  ...
  for (const it of items) {
    ...
    doc.text(fmt(Number(it.total_price)), right - 4, y + 7, { align: 'right' });
...
<p>Veuillez trouver en pièce jointe le reçu de votre commande n° <strong>${orderNumber}</strong> d'un montant de <strong>${fmt(Number(order.total_amount))}</strong>.</p>
<p>Merci pour votre confiance.<br/>SCOLY GROUP SARL · scoly.ci</p>`,
        attachments: [{
      ...
      await sb.from('email_logs').insert({
        ...
        error_message: sent.ok ? null : JSON.stringify(sent).slice(0, 500),
      });
